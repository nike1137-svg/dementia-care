...
## 2026-08-17 — 5주차(기억력): PRD 모순을 근거 조사로 풀다

**PRD 안에 모순이 있었다.** §3.2는 "단어 3개 지연 회상"을 요구하는데 §3.3은 주관식을
배제한다. **자유 회상은 정의상 주관식**이라 양립 불가다. 구현 난이도가 아니라 규칙
충돌이었다. → §3.2-1을 신설해 **단서 회상으로 좁혔다.** 더 쉬운 과제로 바꾼 것이므로
"지연 회상 훈련"이라 부르지 않기로 하고 문서에 명시했다.

**치매 인지중재 기법 14개를 조사했다** (`docs/week5-memory-design.md`). 넷이 설계를
실제로 바꿨다:

1. **Spaced Retrieval** — 인출 간격을 넓혀가며 반복 인출. 우리는 하루 한 세션이라
   **세션 간격이 곧 인출 간격**이다. 세션 안에서 60초 지연을 만들 필요가 없었다.
2. **Errorless Learning** (Wilson·Baddeley 1994) — 오답을 생성하면 그 오답이 함께
   학습된다. **PRD §3.4의 "실패 경험을 주지 마라"는 이탈 방지 이유만이 아니었다.**
   그런데 우리 구현은 문구만 부드럽게 하고 오답 생성은 그대로 뒀다. → 5주차 인출
   문항은 1차 오답 시 재시도 대신 **범주 단서**를 준다(`hint_after_wrong`).
3. **Dual Cognitive Support** (Bäckman) — 부호화·인출 양쪽에 지원이 필요한데 우리는
   부호화 지원이 0이었다.
4. **FCSRT 통제된 부호화** (Grober-Buschke) — 범주로 찾아내게 해 의미 처리를 강제한다.
   **그 행위가 곧 탭 문항**이라 부호화를 쉬운 문항으로 위장할 수 있었다.

**Montessori/CST 때문에 골격을 안 바꿨다.** 절차기억은 비교적 보존되므로 **매일 같은
①~⑤ 순서 자체가 자산**이다. 처음엔 "단계를 늘려야 한다"고 봤는데, 날짜를 건너뛰면
늘릴 필요가 없었다.

**결과: 문항은 전부 static, 문항 관련 서버 코드 변경 0줄.** `day_index=N%5`라 day
순서가 보장되므로 SRT 일정을 파일에 고정하면 정답이 상태에 의존하지 않는다.
앞서 "파일만 추가는 4주차로 끝난다"고 했던 판단을 정정한다.

**검증기를 도구로 만들었다** (`scripts/validate_content.py`). 5주차는 정답이 **문항
사이의 관계**로 정해져서 파일 한 곳만 봐선 맞는지 알 수 없다. `srt_schedule` 표와
실제 문항을 대조한다. 부정 테스트로 11가지 결함을 실제로 잡는 걸 확인했고, 그
과정에서 **검증기 자체 버그**(없는 day 참조 시 KeyError로 죽음)도 나왔다.

**그런데 검증기가 통과시킨 결함을 화면에서 발견했다.** day1 레벨3 부호화 보기의
'항아리'가 day4(주전자) 인출의 오답이었다 — 부호화 때 본 낱말이 친숙해져 나중에
오답으로 고를 확률이 올라가는 간섭이다. 검사가 **같은 항목 안의 간섭만** 보고 있었다.
항목 간까지 보도록 고치고 '거울'로 교체했다.

**교훈: 검증기를 만들었다고 화면 확인을 건너뛰면 안 된다.** 검증기는 내가 예상한
결함만 잡는다. 이번엔 `scripts/demo.sh`로 실제 화면 경로를 훑다가 걸렸다.
